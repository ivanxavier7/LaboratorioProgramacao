...
public class Controller {
     ...
     *   @since		1.0
     */
    public static void lockGrid(Stage primaryStage, GridPane grid, Label logger, Jogador jogador, BigInteger id) {
    	Scene scene = showBottomBar(primaryStage, grid, jogador);
        // Change Events
    	if(MonitorLoto.readJSON() == null) {
	    	if(Grid.validateGrid(grid, logger, id)) {
    ...
    public static void parsePlayerBet(Jogador jogador, Label logger) {
    	...
    	System.out.println("TESTE"+bet);
    	MonitorLoto.writeInfoJSON(Double.toString(bet));
